...
import {
  updateCampaignInfoArr,
  updateProductPricesInShoify,
  setDefaultProductPricesInShoify,
} from "./campaigns.js";
...
//         storeId: session.id,
//         id,
//         // campaignStatus: "Scheduled" || "Expired",
//       },
//     });
//     console.log(
//       "schedule Condition First Start =======",
//       campaign.campaignStatus
//     );

//     if (!campaign.campaignStatus == "Expired" || "Scheduled") return null;
...
export const startJob = async (session, id, campaignStart) => {
  ...
  console.log(
    `StartJob ${startedDate} === 3 ${startedMinute} ${startedHour} ${dd} ${
      startedDate.split("-")[1]
    }`
  );
  try {
            ...
            await db.Campaign.update(
              {
                ...
                campaignMessage:
                  "Already have a campaign actived at this campaign time so end that campaign or start after that campaign end",
              },
              {
                where: {
                  id: id,
...
export const endJob = async (session, id, campaignEnd) => {
  ...
  console.log(
    `endJob ${campaignEndDate1} === 3 ${campaignEndMinute1} ${campaignEndHour1} ${dd} ${
      campaignEndDate1.split("-")[1]
    }`
  );
  try {
    let task = cron.schedule(
      ...
      async () => {
        ...
        if (
          ...
          await setDefaultProductPricesInShoify(session, id);
        } else {
          console.log("************Again schedule endJob ****************");
          await endJob(campaign.id, session, campaign.campaignEnd);
        }
...
export const start = async (session, id) => {
  ...
  try {
    await updateCampaignInfoArr(session, id);
    console.log(
      "@@@@@@@@@@@@@@@@@@@@@@@@@@@@Now Its update the Vales in shopify  Start@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    );
    await updateProductPricesInShoify(session, id);
  } catch (err) {
    console.log("scheduled Start Error", err);
